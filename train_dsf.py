...
from tensorflow.keras.layers import SpatialDropout2D, Dropout, Conv2D, MaxPooling2D, Activation, Conv2DTranspose, concatenate, Input, BatchNormalization
...
train_cfg = {
        "sharpness": 0.23, "k_bound": [0.5, 2.0], "std_bound": [0.25, 1.75],
        "bluriness":  0.23,
        "pertubation": 0.23, "small_scale_bound": [0.9, 1.1], "shift_bound": [-0.1, 0.1],
        "brightness": 0.23, "delta": 0.15,
        "gamma": 0.23, "gamma_bound": [0.5, 1.5],
        "scaling": 0.23, "scale_bound": [0.5, 1.5],
        "shift": 0.23, "width_shift_range": 0.15, "height_shift_range": 0.15,
        "flip": 0.23,
        "rotate": 0.23, "angle": 3.14,
        "distortion": 0.23, "sigma": 4.0, "alpha_bound": [2, 10],
...
save_model_path = os.path.join(os.path.abspath(''), "dsf_last_weights.hdf5")
...
plt.savefig("dsf_last_graph.png")

model = models.load_model(save_model_path, custom_objects={'bce_dice_loss': bce_dice_loss,
                                                           'dice_loss': dice_loss})
scores = model.evaluate(test_input_fn(),
                        steps = int(np.ceil(num_tests / float(test_batch_size))),
                        verbose=1)

text_file = open("Result_dsf_last.txt", "w")
